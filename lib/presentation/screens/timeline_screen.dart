...
class _TimelineScreenState extends ConsumerState<TimelineScreen> {
              ...
              sliver: SliverList(
                delegate: SliverChildListDelegate([
                  const SectionHeader(
                    ...
                    subtitle:
                        'Track your health history across medications, vitals, and follow-ups.',
  ...
  Widget _buildStatisticsCards(
    ...
    TimelineListState timelineState,
  ) {
    ...
    return Column(
      ...
      children: [
        Text(
          ...
          style: Theme.of(
            context,
          ).textTheme.headlineMedium?.copyWith(color: AppTheme.onSurfaceColor),
        ),
        const SizedBox(height: AppSpacing.m),
        LayoutBuilder(
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            if (isSelected)
              Icon(Icons.check, size: 16, color: AppTheme.onPrimaryColor),
    ...
    if (timelineState.items.isEmpty &&
      ...
      return GlassCard(
        ...
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.timeline, size: 64, color: AppTheme.neutralColor),
            const SizedBox(height: AppSpacing.l),
            Text(
      ...
      return Column(
        ...
        children: [
          GlassCard(
            ...
            child: Row(
              children: [
                Icon(Icons.search, size: 20, color: AppTheme.onSurfaceVariant),
          ...
          const SizedBox(height: AppSpacing.l),
          ...sortedGroups.entries.map((entry) {
            try {
          ...
          return GlassCard(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: AppSpacing.s),
                Text('Error: $e', style: Theme.of(context).textTheme.bodySmall),
              ],
            ),
          );
        }
    ...
    showDateRangePicker(
      ...
      if (dateRange != null) {
        ref
            ...
            .loadTimeline(startDate: dateRange.start, endDate: dateRange.end);
      }
    });
  }
        ...
        SnackBar(
          content: Row(
            children: [
              ...
              Expanded(
                child: Text('CSV export ready (${csvData.length} bytes)'),
              ),
            ],
          ),
        ...
        SnackBar(
          content: Row(
            children: [
              ...
              const SizedBox(width: 8),
              Expanded(
                child: Text('JSON export ready (${jsonData.length} bytes)'),
              ),
            ],
          ),
          backgroundColor: Colors.green,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            label,
            style: const TextStyle(fontSize: 12, color: AppTheme.neutralColor),
          ),
        ],
      ),
    );
  ...
  Map<String, int> _calculateStatistics(List<TimelineItem> items) {
    final stats = <String, int>{
      'total': items.length,
      'medicines': items
          .where((item) => item.type == TimelineItemType.medicine)
          .length,
      'prescriptions': items
          .where((item) => item.type == TimelineItemType.prescription)
          .length,
      'followUps': items
          .where((item) => item.type == TimelineItemType.followUp)
          .length,
      'reminderLogs': items
          .where((item) => item.type == TimelineItemType.reminderLog)
          .length,
    };
